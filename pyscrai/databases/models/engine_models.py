...
class EngineState(Base):
    ...
    metadata = Column(JSON)  # Configuration and dynamic state
    
    @property
    def static_config(self):
        """Get static configuration from metadata"""
        return self.metadata.get('static_config', {}) if self.metadata else {}
    
    @property
    def dynamic_state(self):
        """Get dynamic state from metadata"""
        return self.metadata.get('dynamic_state', {}) if self.metadata else {}
